            ...
            {isLoading && (
              <div className="mt-3 space-y-3">
                {Array.from({ length: 4 }).map((_, index) => (
                  <div key={`${section.type}-skeleton-${index}`} className="h-20 animate-pulse rounded-lg bg-slate-100" />
                ))}
              </div>
            )}
            <ul className="mt-3 space-y-3">
                {!isLoading && sectionReferences.map((ref) => (
                <li key={ref.id} className="rounded-lg border border-slate-200 p-3">
                  ...
                  {ref.url && getReferenceHref(ref) && (
                      <a className="break-all text-sm text-indigo-700 underline" href={getReferenceHref(ref) ?? undefined} target="_blank" rel="noreferrer">
                      {ref.url}
                    </a>
                  )}
                  {ref.taxons.length > 0 && (
                      ...
                      {openedReferenceId === ref.id && (
                        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-700">
                          {ref.taxons.map((taxon) => (
                            <li key={taxon.id} className="break-words">{taxon.subfamily} &gt; {taxon.genus} &gt; {taxon.species}</li>
                          ))}
                        </ul>
                      )}
                    </div>
              ...
              {!isLoading && sectionReferences.length === 0 && (
                <li className="rounded-lg border border-slate-200 p-3 text-sm text-slate-500">Aucune référence dans cette section.</li>
              )}
            </ul>
          </section>
        )
