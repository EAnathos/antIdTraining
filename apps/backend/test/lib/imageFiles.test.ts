...
describe('image file helpers', () => {
  ...
  it('deletes base and responsive variants for an uploaded image', () => {
    ...
    expect(rmSpy).toHaveBeenNthCalledWith(1, resolveUploadFilePath('queen.webp'), { force: true })
    expect(rmSpy).toHaveBeenNthCalledWith(2, resolveUploadFilePath('queen-480.webp'), { force: true })
    expect(rmSpy).toHaveBeenNthCalledWith(3, resolveUploadFilePath('queen-960.webp'), { force: true })
  })
})
